...
app.current.app?getQueue():"Playback"===app.current.app&&getQueueMini(c.params.songPos,!0);c.result=c.params;parseUpdateQueue(c);break;case "update_options":getSettings();break;case "update_outputs":sendAPI("MPD_API_PLAYER_OUTPUT_LIST",{},parseOutputs);break;case "update_started":updateDBstarted(!1);break;case "update_database":case "update_finished":updateDBfinished(c.method);updateDBstats();break;case "update_volume":c.result=c.params;parseVolume(c);break;case "update_stored_playlist":"Browse"===
app.current.app&&"Playlists"===app.current.tab&&"All"===app.current.view?sendAPI("MPD_API_PLAYLIST_LIST",{offset:app.current.page,filter:app.current.filter},parsePlaylists):"Browse"===app.current.app&&"Playlists"===app.current.tab&&"Detail"===app.current.view&&sendAPI("MPD_API_PLAYLIST_CONTENT_LIST",{offset:app.current.page,filter:app.current.filter,uri:app.current.search,cols:settings.colsBrowsePlaylistsDetail},parsePlaylists);break;case "update_lastplayed":"Queue"===app.current.app&&"LastPlayed"===
app.current.tab&&sendAPI("MPD_API_QUEUE_LAST_PLAYED",{offset:app.current.page,cols:settings.colsQueueLastPlayed},parseLastPlayed);break;case "error":document.getElementById("alertMpdState").classList.contains("hide")&&showNotification(t(c.params.message),"","","danger");break;case "warn":document.getElementById("alertMpdState").classList.contains("hide")&&showNotification(t(c.params.message),"","","warning");break;case "info":document.getElementById("alertMpdState").classList.contains("hide")&&showNotification(t(c.params.message),
"","","success")}},socket.onclose=function(){logError("Websocket is disconnected");websocketConnected=!1;!0===appInited?(toggleUI(),progressTimer&&clearTimeout(progressTimer)):(showAppInitAlert(t("Websocket connection failed")),logError("Websocket connection failed."));null!==websocketTimer&&(clearTimeout(websocketTimer),websocketTimer=null);websocketTimer=setTimeout(function(){logInfo("Reconnecting websocket");toggleAlert("alertMympdState",!0,t("Websocket connection failed, trying to reconnect")+
...
function parseCollybiaSettings(){document.getElementById("selectDac").value=settings.dac;document.getElementById("selectMixerType").value=settings.mixerType;toggleBtnChk("btnDop",settings.dop);toggleBtnChk("btnFFmpeg",settings.ffmpeg);document.getElementById("selectNsType").value=settings.nsType;document.getElementById("inputNsServer").value=settings.nsServer;document.getElementById("inputNsShare").value=settings.nsShare;document.getElementById("selectSambaVersion").value=settings.sambaVersion;document.getElementById("inputNsUsername").value=
settings.nsUsername;document.getElementById("inputNsPassword").value=settings.nsPassword;0===settings.nsType?(document.getElementById("nsServerShare").classList.add("hide"),document.getElementById("sambaVersion").classList.add("hide"),document.getElementById("nsCredentials").classList.add("hide")):2===settings.nsType?(document.getElementById("nsServerShare").classList.remove("hide"),document.getElementById("sambaVersion").classList.remove("hide"),document.getElementById("nsCredentials").classList.remove("hide")):
(document.getElementById("nsServerShare").classList.remove("hide"),1===settings.nsType?document.getElementById("sambaVersion").classList.remove("hide"):document.getElementById("sambaVersion").classList.add("hide"),document.getElementById("nsCredentials").classList.add("hide"));toggleBtnChk("btnAPMode",settings.apmode);toggleBtnChk("btnAirplay",settings.airplay);toggleBtnChk("btnRoon",settings.roon);toggleBtnChk("btnSpotify",settings.spotify);toggleBtnChkCollapse("btnTidalEnabled","collapseTidal",
settings.tidalEnabled);document.getElementById("inputTidalUsername").value=settings.tidalUsername;document.getElementById("inputTidalPassword").value=settings.tidalPassword;document.getElementById("selectTidalAudioquality").value=settings.tidalAudioquality}
function saveCollybiaSettings(){var a=!0,b=document.getElementById("selectNsType");b=b.options[b.selectedIndex].value;var c=document.getElementById("inputNsServer"),d=document.getElementById("inputNsShare"),f=document.getElementById("inputNsUsername"),g=document.getElementById("inputNsPassword");"0"!==b&&(validateIPAddress(c)||(a=!1),validatePath(d)||(a=!1));"2"===b&&(validateNotBlank(f)&&validateNotBlank(g)||(a=!1));var h=document.getElementById("inputTidalUsername"),k=document.getElementById("inputTidalPassword");
document.getElementById("btnTidalEnabled").classList.contains("active")&&(validateNotBlank(h)&&validateNotBlank(k)||(a=!1));if(!0===a){a=document.getElementById("selectDac");var l=document.getElementById("selectMixerType"),m=document.getElementById("selectSambaVersion"),n=document.getElementById("selectTidalAudioquality");sendAPI("MYMPD_API_SETTINGS_SET",{dac:a.options[a.selectedIndex].value,mixerType:l.options[l.selectedIndex].value,dop:document.getElementById("btnDop").classList.contains("active")?
!0:!1,ffmpeg:document.getElementById("btnFFmpeg").classList.contains("active")?!0:!1,nsType:parseInt(b),nsServer:c.value,nsShare:d.value,sambaVersion:m.options[m.selectedIndex].value,nsUsername:f.value,nsPassword:g.value,apmode:document.getElementById("btnAPMode").classList.contains("active")?!0:!1,airplay:document.getElementById("btnAirplay").classList.contains("active")?!0:!1,roon:document.getElementById("btnRoon").classList.contains("active")?!0:!1,spotify:document.getElementById("btnSpotify").classList.contains("active")?
!0:!1,tidalEnabled:document.getElementById("btnTidalEnabled").classList.contains("active")?!0:!1,tidalUsername:h.value,tidalPassword:k.value,tidalAudioquality:n.options[n.selectedIndex].value},getSettings);modalCollybia.hide()}}
...
function _updateDBfinished(a){var b=document.getElementById("spinnerUpdateProgress");if(b){var c=b.parentNode;b.remove();for(b=0;b<c.children.length;b++)c.children[b].classList.remove("hide")}"update_database"===a?document.getElementById("updateDBfinished").innerText=t("Database successfully updated"):"update_finished"===a&&(document.getElementById("updateDBfinished").innerText=t("Database update finished"));c=document.getElementById("updateDBprogress");c.classList.remove("updateDBprogressAnimate");
...
modalExecScript=new BSN.Modal(document.getElementById("modalExecScript")),modalScripts=new BSN.Modal(document.getElementById("modalScripts")),modalCollybia=new BSN.Modal(document.getElementById("modalCollybia")),modalPartitions=new BSN.Modal(document.getElementById("modalPartitions")),modalPartitionOutputs=new BSN.Modal(document.getElementById("modalPartitionOutputs")),modalTrigger=new BSN.Modal(document.getElementById("modalTrigger")),modalOutputAttributes=new BSN.Modal(document.getElementById("modalOutputAttributes")),
...
document.getElementById("cardBrowseCovergrid").classList.add("hide");setGridPlayback();document.getElementById("card"+app.current.app).classList.remove("hide");document.getElementById("nav"+app.current.app)&&document.getElementById("nav"+app.current.app).classList.add("active");void 0!==app.current.tab&&(document.getElementById("card"+app.current.app+app.current.tab).classList.remove("hide"),document.getElementById("card"+app.current.app+"Nav"+app.current.tab).classList.add("active"));scrollToPosY(a)}(a=
document.getElementById(app.current.app+(void 0===app.current.tab?"":app.current.tab)+(void 0===app.current.view?"":app.current.view)+"List"))?a.classList.add("opacity05"):"Playback"===app.current.app&&(document.getElementById("QueueMiniList").classList.add("opacity05"),document.getElementById("BrowseCovergridList").classList.add("opacity05"))}
...
"",document.getElementById("cardFooterSearch").innerText="",document.getElementById("SearchList").classList.remove("opacity05"),setPagination(0,0));selectTag("searchtags","searchtagsdesc",app.current.filter)}else appGoto("Playback");app.last.app=app.current.app;app.last.tab=app.current.tab;app.last.view=app.current.view}else appGoto("Playback")}}
...
addFilterLetter("BrowsePlaylistsFilterLetters");document.getElementById("syscmds").addEventListener("click",function(a){"A"===a.target.nodeName&&parseCmd(a,a.target.getAttribute("data-href"))},!1);document.getElementById("scripts").addEventListener("click",function(a){"A"===a.target.nodeName&&execScript(a.target.getAttribute("data-href"))},!1);a=document.querySelectorAll("[data-href]");b=a.length;for(c=0;c<b;c++)!1===a[c].classList.contains("notclickable")&&a[c].classList.add("clickable"),!0!==(a[c].parentNode.classList.contains("noInitChilds")?
!0:!1)&&a[c].addEventListener("click",function(a){parseCmd(a,this.getAttribute("data-href"))},!1);a=document.getElementsByClassName("pages");b=a.length;for(c=0;c<b;c++)a[c].addEventListener("click",function(a){"BUTTON"===a.target.nodeName&&gotoPage(a.target.getAttribute("data-page"))},!1);document.getElementById("cardPlaybackTags").addEventListener("click",function(a){"SPAN"===a.target.nodeName&&null!==a.target.parentNode.getAttribute("data-tag")&&gotoBrowse(a.target)},!1);document.getElementById("BrowseBreadcrumb").addEventListener("click",
...
function(a){"A"===a.target.nodeName&&showMenu(a.target,a)},!1);document.getElementById("QueueMiniList").addEventListener("click",function(a){"TD"===a.target.nodeName&&sendAPI("MPD_API_PLAYER_PLAY_TRACK",{track:a.target.parentNode.getAttribute("data-trackid")})},!1);document.getElementById("BrowseFilesystemList").addEventListener("click",function(a){if("TD"===a.target.nodeName)switch(a.target.parentNode.getAttribute("data-type")){case "parentDir":case "dir":appGoto("Browse","Filesystem",void 0,"0/"+
app.current.filter+"/"+app.current.sort+"/"+decodeURI(a.target.parentNode.getAttribute("data-uri")));break;case "song":appendQueue("song",decodeURI(a.target.parentNode.getAttribute("data-uri")),a.target.parentNode.getAttribute("data-name"));break;case "plist":appendQueue("plist",decodeURI(a.target.parentNode.getAttribute("data-uri")),a.target.parentNode.getAttribute("data-name"))}else"A"===a.target.nodeName&&showMenu(a.target,a)},!1);document.getElementById("BrowseCovergridAlbumList").addEventListener("click",
...
a.target.parentNode.getAttribute("data-name")):"A"===a.target.nodeName&&showMenu(a.target,a)},!1);document.getElementById("BrowseDatabaseTagList").addEventListener("click",function(a){"TD"===a.target.nodeName&&appGoto("Browse","Database",app.current.view,"0/-/-/"+decodeURI(a.target.parentNode.getAttribute("data-uri")))},!1);document.getElementById("SearchList").addEventListener("click",function(a){"TD"===a.target.nodeName?appendQueue("song",decodeURI(a.target.parentNode.getAttribute("data-uri")),
...
"0/"+app.current.filter+"/"+app.current.sort+"/"+app.current.search)},!1);document.getElementById("inputScriptArgument").addEventListener("keyup",function(a){"Enter"===a.key&&(a.preventDefault(),a.stopPropagation(),addScriptArgument())},!1);document.getElementById("selectScriptArguments").addEventListener("click",function(a){"OPTION"===a.target.nodeName&&removeScriptArgument(a)},!1);document.getElementsByTagName("body")[0].addEventListener("click",function(){hideMenu()},!1);dragAndDropTable("QueueCurrentList");
...
function gotoAlbumList(){document.getElementById("btnBrowseCovergridAlbum").parentNode.classList.add("hide");getBrowseCovergrid()}
...
function updateDBstats(){sendAPI("MPD_API_DATABASE_STATS",{},parseDBstats)}function parseDBstats(a){document.getElementById("panel-heading-browse").innerHTML="Library total: Tracks "+a.result.songs+" &bull; Time "+beautifyDuration(a.result.dbPlaytime)}
...
function showMenuTd(a){var b=a.getAttribute("data-type"),c=decodeURI(a.getAttribute("data-uri")),d=decodeURI(a.getAttribute("data-name")),f=0;if(null===b||""===c)b=a.parentNode.parentNode.getAttribute("data-type"),c=decodeURI(a.parentNode.parentNode.getAttribute("data-uri")),d=a.parentNode.parentNode.getAttribute("data-name");lastState&&(f=lastState.nextSongPos);var g="";if("Browse"===app.current.app&&"Filesystem"===app.current.tab||"Search"===app.current.app||"Browse"===app.current.app&&"Database"===
app.current.tab||"Browse"===app.current.app&&"Covergrid"===app.current.tab&&"A"===a.nodeName||"Playback"===app.current.app&&"A"===a.nodeName)g+=addMenuItem({cmd:"appendQueue",options:[b,c,d]},t("Append to queue"))+("song"===b?addMenuItem({cmd:"appendAfterQueue",options:[b,c,f,d]},t("Add after current playing song")):"")+addMenuItem({cmd:"replaceQueue",options:[b,c,d]},t("Replace queue"))+("plist"!==b&&"smartpls"!==b&&settings.featPlaylists?addMenuItem({cmd:"showAddToPlaylist",options:[c,""]},t("Add to playlist")):
"")+("song"===b?addMenuItem({cmd:"songDetails",options:[c]},t("Song details")):"")+("plist"===b||"smartpls"===b?addMenuItem({cmd:"playlistDetails",options:[c]},t("View playlist")):"")+("dir"===b&&settings.featBookmarks?addMenuItem({cmd:"showBookmarkSave",options:[-1,d,c,b]},t("Add bookmark")):""),"Filesystem"===app.current.tab&&(g+=("dir"===b?addMenuItem({cmd:"updateDB",options:[dirname(c),!0]},t("Update directory")):"")+("dir"===b?addMenuItem({cmd:"rescanDB",options:[dirname(c),!0]},t("Rescan directory")):
"")),"Search"===app.current.app&&(c=dirname(c),g+='<div class="dropdown-divider"></div><a class="dropdown-item" id="advancedMenuLink" data-toggle="collapse" href="#advancedMenu"><span class="material-icons material-icons-small-left">keyboard_arrow_right</span>Album actions</a><div class="collapse" id="advancedMenu">'+addMenuItem({cmd:"appendQueue",options:[b,c,d]},t("Append to queue"))+addMenuItem({cmd:"appendAfterQueue",options:[b,c,f,d]},t("Add after current playing song"))+addMenuItem({cmd:"replaceQueue",
...
options:[c]},t("Edit playlist")))+(!0===settings.smartpls&&"smartpls"===b?addMenuItem({cmd:"showSmartPlaylist",options:[c]},t("Edit smart playlist")):"")+(!0===settings.smartpls&&"smartpls"===b?addMenuItem({cmd:"updateSmartPlaylist",options:[c]},t("Update smart playlist")):"")+addMenuItem({cmd:"showRenamePlaylist",options:[c]},t("Rename playlist"))+addMenuItem({cmd:"showDelPlaylist",options:[c]},t("Delete playlist"));else if("Browse"===app.current.app&&"Playlists"===app.current.tab&&"Detail"===app.current.view)f=
document.getElementById("BrowsePlaylistsDetailList"),g+=addMenuItem({cmd:"appendQueue",options:[b,c,d]},t("Append to queue"))+addMenuItem({cmd:"replaceQueue",options:[b,c,d]},t("Replace queue"))+("false"===f.getAttribute("data-ro")?addMenuItem({cmd:"removeFromPlaylist",options:[f.getAttribute("data-uri"),a.parentNode.parentNode.getAttribute("data-songpos")]},t("Remove")):"")+(settings.featPlaylists?addMenuItem({cmd:"showAddToPlaylist",options:[c,""]},t("Add to playlist")):"")+(-1===c.indexOf("http")?
addMenuItem({cmd:"songDetails",options:[c]},t("Song details")):"");else if("Queue"===app.current.app&&"Current"===app.current.tab)g+=addMenuItem({cmd:"delQueueSong",options:["single",a.parentNode.parentNode.getAttribute("data-trackid")]},t("Remove"))+addMenuItem({cmd:"delQueueSong",options:["range",0,a.parentNode.parentNode.getAttribute("data-songpos")]},t("Remove all upwards"))+addMenuItem({cmd:"delQueueSong",options:["range",parseInt(a.parentNode.parentNode.getAttribute("data-songpos"))-1,-1]},
t("Remove all downwards"))+(-1===c.indexOf("http")?addMenuItem({cmd:"songDetails",options:[c]},t("Song details")):"");else if("Queue"===app.current.app&&"LastPlayed"===app.current.tab)g+=addMenuItem({cmd:"appendQueue",options:[b,c,d]},t("Append to queue"))+addMenuItem({cmd:"replaceQueue",options:[b,c,d]},t("Replace queue"))+(settings.featPlaylists?addMenuItem({cmd:"showAddToPlaylist",options:[c,""]},t("Add to playlist")):"")+(-1===c.indexOf("http")?addMenuItem({cmd:"songDetails",options:[c]},t("Song details")):
"");else if("Browse"===app.current.app&&"Covergrid"===app.current.tab&&"DIV"===a.nodeName||"Playback"===app.current.app&&"DIV"===a.nodeName)b=decodeURI(a.parentNode.getAttribute("data-album")),d=decodeURI(a.parentNode.getAttribute("data-albumartist")),b="((Album == '"+b+"') AND (AlbumArtist == '"+d+"'))",d=a.parentNode.getElementsByClassName("card-body")[0].getAttribute("id"),g+=addMenuItem({cmd:"getCovergridTitleList",options:[d]},t("Show songs"))+addMenuItem({cmd:"addAllFromSearchPlist",options:["queue",
b,!1]},t("Append to queue"))+addMenuItem({cmd:"addAllFromSearchPlist",options:["queue",b,!0]},t("Replace queue"))+(settings.featPlaylists?addMenuItem({cmd:"showAddToPlaylist",options:["ALBUM",b]},t("Add to playlist")):"");new BSN.Popover(a,{trigger:"click",delay:0,dismissible:!0,template:'<div class="popover" role="tooltip"><div class="arrow"></div><div class="popover-content">'+g+"</div></div>",content:" "});g=a.Popover;null===a.getAttribute("data-init")&&(a.setAttribute("data-init","true"),a.addEventListener("shown.bs.popover",
...
!1),new BSN.Collapse(b));document.getElementsByClassName("popover-content")[0].firstChild.focus()},!1));g.show()}
...
app.current.app?appRoute():"Browse"===app.current.app&&"Filesystem"===app.current.tab?appRoute():"Browse"===app.current.app&&"Playlists"===app.current.tab&&"Detail"===app.current.view?appRoute():"Browse"===app.current.app&&"Database"===app.current.tab&&""!==app.current.search?appRoute():"Playback"===app.current.app&&getBrowseCovergrid();i18nHtml(document.getElementsByTagName("body")[0]);checkConsume();!0===settings.mediaSession&&"mediaSession"in navigator?(navigator.mediaSession.setActionHandler("play",
...
logDebug("mediaSession not supported by browser");parseCollybiaSettings();settingsParsed="true"}
...
if(!0===b){b=document.getElementById("selectLocale");var n=document.getElementById("selectTheme");sendAPI("MYMPD_API_SETTINGS_SET",{consume:document.getElementById("btnConsume").classList.contains("active")?1:0,random:document.getElementById("btnRandom").classList.contains("active")?1:0,single:parseInt(g),repeat:document.getElementById("btnRepeat").classList.contains("active")?1:0,replaygain:k,crossfade:document.getElementById("inputCrossfade").value,mixrampdb:!0===settings.featMixramp?document.getElementById("inputMixrampdb").value:
...
" - Windows Media Audio";default:return a}}function fileformat(a){return a.bits+t("bits")+" - "+a.sampleRate/1E3+t("kHz")}function scrollToPosY(a){document.body.scrollTop=a;document.documentElement.scrollTop=a;"Playback"===app.current.app&&(document.getElementById("BrowseCovergridBox").scrollTop=0)}
...
function setPagination(a,b,c){c=void 0===c?settings.maxElementsPerPage:c;var d="Playback"===app.current.app?"BrowseCovergrid":app.current.app+app.current.tab;"Search"===app.current.app&&(d=app.current.app+(void 0===app.current.tab?"":app.current.tab));var f=Math.ceil(a/c);0===f&&(f=1);for(var g=[document.getElementById(d+"PaginationTop"),document.getElementById(d+"PaginationBottom")],h=0;h<g.length;h++){var k=g[h].children[0],l=g[h].children[1].children[0],m=g[h].children[1].children[1],n=g[h].children[2];
l.innerText=app.current.page/c+1+" / "+f;if(1<f){l.removeAttribute("disabled");for(var p="",q=0;q<f;q++)p+='<button data-page="'+q*c+'" type="button" class="mr-1 mb-1 btn-sm btn btn-secondary">'+(q+1)+"</button>";m.innerHTML=p;l.classList.remove("nodropdown")}else-1===a?(l.setAttribute("disabled","disabled"),l.innerText=app.current.page/c+1):l.setAttribute("disabled","disabled"),l.classList.add("nodropdown");a>app.current.page+c||-1===a&&b>=c?(n.removeAttribute("disabled"),g[h].classList.remove("hide"),
document.getElementById(d+"ButtonsBottom").classList.remove("hide")):(n.setAttribute("disabled","disabled"),g[h].classList.add("hide"),document.getElementById(d+"ButtonsBottom").classList.add("hide"));0<app.current.page?(k.removeAttribute("disabled"),g[h].classList.remove("hide"),document.getElementById(d+"ButtonsBottom").classList.remove("hide")):k.setAttribute("disabled","disabled")}}function genId(a){return"id"+a.replace(/[^\w-]/g,"")}
...
function gotoPage(a){switch(a){case "next":app.current.page+=settings.maxElementsPerPage;break;case "prev":app.current.page-=settings.maxElementsPerPage;0>app.current.page&&(app.current.page=0);break;default:app.current.page=a}setAppState(app.current.page,app.current.filter,app.current.sort,app.current.search);appGoto(app.current.app,app.current.tab,app.current.view,app.current.page+"/"+app.current.filter+"/"+app.current.sort+"/"+app.current.search)}
