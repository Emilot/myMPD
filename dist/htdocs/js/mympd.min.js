...
document.getElementById("inputNsPassword").classList.remove("is-invalid")});document.getElementById("modalCollybia").addEventListener("keydown",function(a){"Enter"===a.key&&(saveCollybiaSettings(),a.stopPropagation(),a.preventDefault())});document.getElementById("selectNsType").addEventListener("change",function(){var a=this.options[this.selectedIndex].value;"0"===a?(document.getElementById("nsServerShare").classList.add("hide"),document.getElementById("sambaVersion").classList.add("hide"),document.getElementById("nsCredentials").classList.add("hide"),
document.getElementById("inputNsServer").value="",document.getElementById("inputNsShare").value="",document.getElementById("inputNsUsername").value="",document.getElementById("inputNsPassword").value=""):"2"===a?(document.getElementById("nsServerShare").classList.remove("hide"),document.getElementById("sambaVersion").classList.remove("hide"),document.getElementById("nsCredentials").classList.remove("hide")):(document.getElementById("nsServerShare").classList.remove("hide"),"1"===a?document.getElementById("sambaVersion").classList.remove("hide"):
...
socket=null);websocketConnected=!1});document.getElementById("localPlayer").addEventListener("canplay",function(){document.getElementById("alertLocalPlayback").classList.add("hide");!0===settings.featLocalplayer&&!0===settings.localplayerAutoplay&&localplayerPlay()});updateDBstats();checkForUpdates()}
...
b,!1]},t("Append to queue"))+addMenuItem({cmd:"addAllFromSearchPlist",options:["queue",b,!0]},t("Replace queue"))+(settings.featPlaylists?addMenuItem({cmd:"showAddToPlaylist",options:["ALBUM",b]},t("Add to playlist")):"");new Popover(a,{trigger:"click",delay:0,dismissible:!0,template:'<div class="popover" role="tooltip"><div class="arrow"></div><div class="popover-content">'+g+"</div></div>",content:" "});g=a.Popover;a.setAttribute("data-init","true");a.addEventListener("shown.bs.popover",function(a){a.target.setAttribute("data-popover",
"true");document.getElementsByClassName("popover-content")[0].addEventListener("click",function(b){b.preventDefault();b.stopPropagation();"A"===b.target.nodeName&&(b=b.target.getAttribute("data-href"))&&(b=JSON.parse(b64DecodeUnicode(b)),parseCmd(a,b),hideMenu())},!1);document.getElementsByClassName("popover-content")[0].addEventListener("keydown",function(a){a.preventDefault();a.stopPropagation();if("ArrowDown"===a.key||"ArrowUp"===a.key){var b=this.getElementsByTagName("a");b=Array.prototype.slice.call(b);
var c=b.indexOf(document.activeElement);do if(c="ArrowUp"===a.key?1<c?c-1:0:"ArrowDown"===a.key?c<b.length-1?c+1:c:c,0===c||c===b.length-1)break;while(!b[c].offsetHeight);b[c]&&b[c].focus()}else"Enter"===a.key?a.target.click():"Escape"===a.key&&hideMenu()},!1);var b=document.getElementById("advancedMenuLink");b&&(b.addEventListener("click",function(){var a=this.getElementsByTagName("span")[0];a.innerText="keyboard_arrow_right"===a.innerText?"keyboard_arrow_down":"keyboard_arrow_right"},!1),new Collapse(b));
document.getElementsByClassName("popover-content")[0].firstChild.focus()},!1);g.show()}
...
seekRelativeForward),navigator.mediaSession.setActionHandler("previoustrack",clickPrev),navigator.mediaSession.setActionHandler("nexttrack",clickNext),navigator.mediaSession.setPositionState||logDebug("mediaSession.setPositionState not supported by browser")):logDebug("mediaSession not supported by browser");parseCollybiaSettings();settingsParsed="true"}
...
var n=document.getElementById("selectTheme");sendAPI("MYMPD_API_SETTINGS_SET",{consume:document.getElementById("btnConsume").classList.contains("active")?1:0,random:document.getElementById("btnRandom").classList.contains("active")?1:0,single:parseInt(g),repeat:document.getElementById("btnRepeat").classList.contains("active")?1:0,replaygain:k,crossfade:document.getElementById("inputCrossfade").value,mixrampdb:!0===settings.featMixramp?document.getElementById("inputMixrampdb").value:settings.mixrampdb,
