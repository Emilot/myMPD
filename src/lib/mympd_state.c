...
void mympd_state_default(struct t_mympd_state *mympd_state, struct t_config *config) {
    ...
    mympd_state->mixer = sdsnew(MYMPD_MIXER);
    mympd_state->dac = sdsnew(MYMPD_DAC);
    mympd_state->apmode = MYMPD_APMODE;
    mympd_state->dop = MYMPD_DOP;
    mympd_state->ffmpeg = MYMPD_FFMPEG;
    mympd_state->roon = MYMPD_ROON;
    mympd_state->airplay = MYMPD_AIRPLAY;
    mympd_state->spotify = MYMPD_SPOTIFY;
    mympd_state->wifi = MYMPD_WIFI;
    mympd_state->samba_version = sdsnew(MYMPD_SAMBA_VERSION);
    mympd_state->ns_server = sdsnew(MYMPD_NS_SERVER);
    mympd_state->ns_share = sdsnew(MYMPD_NS_SHARE);
    mympd_state->ns_username = sdsnew(MYMPD_NS_USERNAME);
    mympd_state->ns_password = sdsnew(MYMPD_NS_PASSWORD);
    mympd_state->wifissid = sdsnew(MYMPD_WIFISSID);
    mympd_state->wifi_password = sdsnew(MYMPD_WIFI_PASSWORD);
